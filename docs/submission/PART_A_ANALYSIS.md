...
| Where could I use this in my own project? | A notification service, payment provider, export format, or task type can hide variant construction behind one stable API. StudyFlow uses the same idea for Assignment, Exam, and Project products. | `04-studyflow/patterns/factory/TaskFactory.ts:56-75` |

## MVC (§7.3.4)

| Question | Answer | Evidence |
|---|---|---|
| Which files are Model, View, and Controller? | Model: `models/Task.ts`; View: `components/TaskForm.tsx` and `components/TaskList.tsx`; Controller: `controllers/TaskController.ts`. API routes connect HTTP requests to the controller. | `03-mvc/models/Task.ts:12-38`; `03-mvc/components/TaskForm.tsx`; `03-mvc/components/TaskList.tsx`; `03-mvc/controllers/TaskController.ts:15-58` |
