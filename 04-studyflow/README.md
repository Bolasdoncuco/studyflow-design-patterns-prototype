...
The model is the immutable task state and transitions in `domain/Task.ts` and `domain/TaskModel.ts:10-41`. The controller coordinates validation, Factory creation, Strategy ranking, filtering, and persistence in `controllers/useTaskController.ts:24-83`. The View is the React component tree in `app/page.tsx:18-21` and `components/`. This keeps UI rendering separate from domain transitions and application coordination.
